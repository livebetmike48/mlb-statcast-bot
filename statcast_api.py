...
def get_todays_probable_pitchers(date_str: str) -> list[dict]:
    """
    Every probable starting pitcher currently in a LIVE game today, with
    their game_pk and team name -- the scan list the auto-post loop checks
    each cycle instead of requiring someone to run /livevelo manually.
    """
    resp = requests.get(
        f"{MLB_BASE}/schedule",
    ...
    for date_entry in data.get("dates", []):
        for g in date_entry.get("games", []):
            if g.get("status", {}).get("abstractGameState") != "Live":
                continue  # only worth live-checking games actually in progress
